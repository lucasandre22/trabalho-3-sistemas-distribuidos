...
PERIOD_IN_SECONDS_TO_NOTIFY_CLIENT_PRODUCT_NOT_BEING_SOLD: int = 20
...
class Server:
    ...
    def __init__(self):
        # Start thread to monitor products
        thread = threading.Thread(target=self.start_monitoring_products_not_being_sold) #test
        thread.start()
    ...
    def verify_signature(self, json_product):
        try:
            ...
            # Restore the key for the current user and use it to verify the message
    ...
    def notify_client_product_minimum_stock(self, product):
        print("Product reached minimum stock! Notifying Client.")
    ...
    def notify_client_product_not_being_sold(self, products):
        print("There are products not being sold! Notifying Client.")
        client_remote_uri = self.users.get(current_user).get_uri()
        client = Pyro5.api.Proxy(client_remote_uri)
    ...
    def start_monitoring_products_not_being_sold(self) -> [str]:
        import time
        print("Starting to monitor products...")
        while True:
        ...
        if self.verify_signature(json_product):
            ...
            for product in self.products:
                if product['code'] == product_code:
                    ...
                    print(f"Added {quantity_to_store} units to existing product with code {product_code}.")
                    print(self.products)
                    break
            else:
    ...
    def subtract_product(self, json_product):
        """
        Subtract a given product from stock.
